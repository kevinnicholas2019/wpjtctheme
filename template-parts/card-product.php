    ...
    <a href="<?= get_permalink($product->id) ?>" class="card card-product-jtc rounded jtc-fixed-height-card">
        ...
        <div class="p-2 card-detail">
            ...
            <?php if ($product->sale_price) { ?>
                <h4 class="card-diskon"><span class="text-decoration-line-through"><?= do_shortcode("[money-format price='$product->sale_price']") ?></span> <span class="percentage-diskon text-danger"><?= round(((($product->regular_price / $product->sale_price) - 1) * 100)) . '%' ?></span></h4>
            <?php } ?>
